...
--      of birth using Postgres's own calendar-correct age(), never the
...
--
--   3. SUBMIT_DOB_ELIGIBILITY — the sole write path for (1). Validates
--      a real, non-future, non-implausible calendar date; computes age
--      via age(); records `eligible` (with the exact DOB retained — see
--      below) or `ineligible` (DOB NOT retained; only the future
--      re-screening date is). Re-synchronizes an existing profile's
--      derived age_range from the newly-confirmed DOB.
--
--   4. ACCEPT_CURRENT_LEGAL_DOCUMENTS — the sole write path for (2).
...
--
--   5. TEMPA_PRIVATE.DERIVE_AGE_RANGE — the ONE canonical age-range
...
--
--   6. PROFILES_ENFORCE_ADULT_ELIGIBILITY — a BEFORE INSERT trigger,
--      the same shape as this table's own existing
--      profiles_force_initial_mark_stage trigger (docs/sql/2026-09-29-
--      your-mark-production.sql): defense-in-depth so a profile can
...
--
-- UNDER-18 HANDLING — an ineligible submission does NOT retain the
-- submitted DOB in the ordinary date_of_birth field (it is written as
-- NULL); only `eligible_on` (date_of_birth + 18 years, calendar-
-- correct) survives, so the system can permit a fresh, neutral
-- screening once that date arrives WITHOUT permanently retaining a
-- minor's exact birth date as an ordinary account DOB. Before
-- eligible_on, submit_dob_eligibility returns the EXISTING persisted
-- ineligible decision without accepting or evaluating a replacement
-- DOB at all — an immediate resubmission cannot turn this into
...
create table public.account_eligibility (
  ...
  -- which a fresh, neutral DOB screening is permitted again.
...
-- ============================================================
-- 3. TEMPA_PRIVATE.DERIVE_AGE_RANGE — the one canonical bucket rule
-- ============================================================
-- Mirrors lib/age.ts's deriveAgeRangeBucket exactly (six buckets:
-- 18-24, 25-34, 35-44, 45-54, 55-64, 65+). Intentionally duplicated in
-- SQL — documented in both places — because a Postgres function cannot
...
as $$
  select case
    when p_dob is null then null
      ...
      from (select extract(year from age(current_date, p_dob))::int as age) a
    )
  end
$$;
...
-- ============================================================
-- 4. SUBMIT_DOB_ELIGIBILITY — sole write path for account_eligibility
-- ============================================================
...
begin
  ...
  if extract(year from age(current_date, v_dob)) > 120 then
    raise exception 'Enter a valid date.';
  end if;
  ...
  -- is NOT reopened by a fresh submission — returns the EXISTING
  -- decision unchanged, without evaluating or accepting the new DOB at
  -- all. This is what prevents "wrong answer, try again" DOB retries
  ...
  if v_age >= 18 then
    ...
    -- eligible_on = dob + 18 years, calendar-correct. A February 29
    -- birth resolves to February 28 in its (necessarily non-leap —
    -- 18 is not a multiple of 4, so dob.year and dob.year + 18 can
    -- never both be leap years) target year, matching lib/age.ts's
    -- eligibleOnDate exactly.
    v_target_year := extract(year from v_dob)::int + 18;
    if extract(month from v_dob) = 2 and extract(day from v_dob) = 29 then
      v_eligible_on := make_date(v_target_year, 2, 28);
    else
      v_eligible_on := make_date(v_target_year, extract(month from v_dob)::int, extract(day from v_dob)::int);
    end if;

    -- The submitted DOB is deliberately NOT retained here — see this
    -- migration's own header, "UNDER-18 HANDLING."
    insert into public.account_eligibility (user_id, date_of_birth, status, eligible_on, checked_at, updated_at)
...
-- ============================================================
-- 5. ACCEPT_CURRENT_LEGAL_DOCUMENTS — sole write path for legal_acceptances
-- ============================================================
...
-- ============================================================
-- 6. PROFILES_ENFORCE_ADULT_ELIGIBILITY — defense in depth
-- ============================================================
-- Same trigger SHAPE as this table's own existing
-- profiles_force_initial_mark_stage (docs/sql/2026-09-29-your-mark-
-- production.sql): BEFORE INSERT, security invoker (runs as the
-- inserting member's own privileges — account_eligibility's RLS
-- already lets a member read their OWN row, which is all this needs),
-- fixed search_path. A profile can never be created for an account
-- that is not currently `eligible`, regardless of what the client
-- attempts directly against the table — this does not replace /begin's
-- own UI gate, it backstops it. age_range is force-overwritten from
-- the authoritative DOB every time, exactly like onboarding_stage/
-- mark_id are force-overwritten by the existing sibling trigger —
-- whatever (if anything) the client supplied for age_range is ignored.
