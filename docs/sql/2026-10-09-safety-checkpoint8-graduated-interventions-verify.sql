...
intervention_semantics_check as (
  select
    ...
    -- INDEPENDENT AUDIT CORRECTION: the actual fix — this function locks
    -- the subject's own profiles row (an always-existing row) rather
    -- than trying to lock account_enforcement_state itself (which may
    -- have no row at all for a never-before-enforced member, so FOR
    -- UPDATE there locks nothing and cannot serialize anything).
    coalesce(
      pg_get_functiondef(p.oid) ilike '%from public.profiles where id = v_case.subject_user_id for update%',
      false
    ) as locks_subject_profile_row,
    coalesce(
...
-- The redefined admin_set_account_status (Part 0 of the migration)
-- must ALSO lock the target's own profiles row, before it ever reads/
-- upserts account_enforcement_state — this is the other half of the
-- shared-lock fix; without this half, the ordinary member-workspace
-- path would still race unprotected against the case-intervention path.
set_account_status_semantics_check as (
  select
    coalesce(
      pg_get_functiondef(p.oid) ilike '%from public.profiles where id = p_user_id for update%',
      false
    ) as locks_target_profile_row,
    coalesce(pg_get_functiondef(p.oid) ilike '%staff accounts must be managed separately%', false) as still_protects_staff_accounts,
    coalesce(pg_get_functiondef(p.oid) ilike '%''set_account_status''%', false) as still_writes_own_audit_row
  from (select 1 as anchor) _anchor
  left join pg_proc p on p.oid = to_regprocedure('public.admin_set_account_status(uuid, text, text)')
),
...
select
  ...
  isc.locks_case_row, isc.locks_subject_profile_row, isc.null_safe_staleness_checks_both_axes,
  isc.restricted_to_three_terminal_statuses,
  ...
  sas.locks_target_profile_row, sas.still_protects_staff_accounts, sas.still_writes_own_audit_row,
  ...
  (
    ...
    and isc.locks_case_row and isc.locks_subject_profile_row and isc.null_safe_staleness_checks_both_axes
    and isc.restricted_to_three_terminal_statuses
    ...
    and sas.locks_target_profile_row and sas.still_protects_staff_accounts and sas.still_writes_own_audit_row
...
from signatures_present_check ss, grant_check g, staff_gate_check sg, intervention_semantics_check isc,
     set_account_status_semantics_check sas, no_direct_grant_check ng, no_automatic_wiring_check naw;
