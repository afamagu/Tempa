...
export default function MomentsComposer({
  ...
  // the Dispatch composer's own postcardIntroActive: showPostcardIntro
  // (still-unseen, per guide_completions) used to be enough on its own
  // to render the introduction the instant this composer mounted. It
  // now only appears once the member actually activates the slot — see
  // handleAddPostcard below.
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Post-onboarding corrections checkpoint (Section G/H/I) — shown
          only once the member actually activates the Postcard slot
          below (handleAddPostcard) and only once it's actually usable
          (momentsQualified), never pre-emptively on composer mount. Its
          CTA both completes the guide AND opens the real picker,
          immediately. Same shared 'postcard' guide key as the Dispatch
          composer's own introduction; completing/dismissing it on
          either surface prevents it reappearing on the other. */}
      {momentsQualified && postcardIntroActive && !postcardDraft && (
        <FeatureIntroduction
          guideKey="postcard"
          title="Postcards"
        ...
        >
          ...
          <p>
            Choose a postcard, add a few words to the front, and write something more on the
            back — then send it along with your Letter or Dispatch as a small keepsake.
          </p>
        </FeatureIntroduction>
      )}
