...
function profileHref(userId: string, returnTo: string) {
  return returnTo ? `/minds/${userId}?returnTo=${encodeURIComponent(returnTo)}` : `/minds/${userId}`
}
...
export default function DiscoveryResults({ entries, returnTo = '/minds' }: { entries: DiscoveryEntry[]; returnTo?: string }) {
        ...
        {entries.map((entry) => (
          <div key={entry.userId} className="rounded-md border border-foreground/10 p-4">
            <div className="flex items-start gap-3">
              <button type="button" onClick={() => setOpenId(entry.userId)} className="flex min-w-0 flex-1 items-center gap-3 text-left hover:opacity-90" aria-label={`Read ${entry.pseudonym}'s response`}>
          ...
          <div role="dialog" aria-modal="true" aria-label={`${openEntry.pseudonym}'s response`} onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd} className="relative flex w-full flex-col overflow-hidden bg-background sm:h-auto sm:max-h-[85vh] sm:w-full sm:max-w-xl sm:rounded-lg sm:border sm:border-foreground/10">
            ...
            <div className="space-y-3 border-t border-foreground/10 px-5 py-4">
              <div className="flex items-center justify-between gap-3">
                ...
                <button type="button" onClick={() => router.push(`/write/${openEntry.userId}?a=${openEntry.response.id}`)} className={primaryButtonClass}>Write to {openEntry.pseudonym}</button>
              </div>
              <Link href={profileHref(openEntry.userId, returnTo)} className={quietLinkClass}>View {openEntry.pseudonym}&rsquo;s profile</Link>
            </div>
          </div>
        </div>
      )}
